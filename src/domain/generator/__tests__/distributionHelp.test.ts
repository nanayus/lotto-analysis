...
describe('buildDistributionHelp', () => {
  test('summarizes the most frequent distribution values from bundled main numbers', () => {
    ...
    expect(help.standardDeviation).toMatchObject({
      ...
      historicalCount: 195,
      historicalPercentage: 15.8,
    });
    expect(help.sum).toMatchObject({ historicalLabel: '130~139', historicalCount: 167 });
    expect(help.lastDigitSum).toMatchObject({ historicalLabel: '25~29', historicalCount: 359 });
    expect(help.oddEven).toMatchObject({ historicalLabel: '3:3', historicalCount: 413 });
    expect(help.lowHigh).toMatchObject({ historicalLabel: '3:3', historicalCount: 407 });
    ...
    expect(help.primeCount).toMatchObject({ historicalLabel: '2개', historicalCount: 444 });
    expect(help.squareCount).toMatchObject({ historicalLabel: '0개', historicalCount: 620 });
    ...
    expect(help.consecutivePattern).toMatchObject({ historicalLabel: '없음', historicalCount: 598 });
    expect(help.band40To45).toMatchObject({ historicalLabel: '1개', historicalCount: 525 });
    expect(help.pastRanks.historicalHeading).toBe('비교 기준');
    expect(help.sameEnding.sourceLabel).toBe('1~1,237회 본번호 · 1,237개 조합');
  });
});
