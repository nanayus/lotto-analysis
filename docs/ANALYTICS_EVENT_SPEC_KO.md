...
| `analysis_result_viewed` | 분석 결과 화면 진입 | `access_method`, `period`, `bonus_included`, 조합 지표 |
...
`combination_key`는 값 종류가 매우 많으므로 GA4의 커스텀 측정기준으로 등록하지 않는다. 개별 조합 순위가 필요하면 Firebase의 BigQuery 내보내기를 연결해 원시 이벤트를 집계한다. GA4 대시보드에는 `source`, `account_tier`, `access_method`, `period`, `generation_mode`, `reason`, `intent`처럼 값 종류가 적은 항목만 커스텀 측정기준으로 등록한다.
