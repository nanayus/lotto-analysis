...
- 현재 등급만 `FREE` 또는 `PRO`로 명확하게 표시한다. 두 등급을 동시에 탭처럼 배치하지 않는다.
- FREE는 중립 표면, PRO는 강조 표면과 강조 글자색으로 현재 상태를 구분한다. 강한 테두리와 단색 채움은 사용하지 않는다.
- FREE 배지는 기존 공통 Pro 안내를 열고, PRO 배지는 이용 기간 정보창을 연다.
- Pro 만료일은 `proExpiresAt`이 유효할 때만 날짜로 표시한다.
